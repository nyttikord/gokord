...
func ListMessages(channelID uint64, limit int, beforeID, afterID, aroundID uint64) Request[[]*Message] {
	...
	if afterID != 0 {
		v.Set("after", fmt.Sprintf("%d", afterID))
	}
	if beforeID != 0 {
		v.Set("before", fmt.Sprintf("%d", beforeID))
	}
	if aroundID != 0 {
		v.Set("around", fmt.Sprintf("%d", aroundID))
	}
	if len(v) > 0 {
		uri += "?" + v.Encode()
	}
...
func GetMessage(channelID, messageID uint64) Request[*Message] {
	return NewData[*Message](http.MethodGet, discord.EndpointChannelMessage(channelID, messageID)).
		WithBucketID(discord.EndpointChannelMessage(channelID, 0))
}

// EditMessage, replacing it entirely with the given content.
func EditMessage(channelID, messageID uint64, content string) Request[*Message] {
...
func EditMessageComplex(m *MessageEdit) Request[*Message] {
	...
	if len(m.Files) == 0 {
		return NewData[*Message](http.MethodPatch, endpoint).
			WithBucketID(discord.EndpointChannelMessage(m.Channel, 0)).WithData(m)
	}
	return NewMultipart[*Message](http.MethodPatch, endpoint, m, m.Files).
		WithBucketID(discord.EndpointChannelMessage(m.Channel, 0))
}

// DeleteMessage from the given [Channel].
func DeleteMessage(channelID, messageID uint64) Empty {
...
func DeleteMessages(channelID uint64, messages []uint64) Empty {
	if len(messages) == 0 {
		// to do nothing
