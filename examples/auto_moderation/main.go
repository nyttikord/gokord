...
func main() {
	...
	enabled := true
	rule, err := session.GuildAPI().AutoModerationRuleCreate(context.Background(), *GuildID, &guild.AutoModerationRule{
	...
	defer session.GuildAPI().AutoModerationRuleDelete(context.Background(), *GuildID, rule.ID)

	session.EventManager().AddHandlerOnce(func(ctx context.Context, s bot.Session, e *event.AutoModerationActionExecution) {
		_, err = session.GuildAPI().AutoModerationRuleEdit(ctx, *GuildID, rule.ID, &guild.AutoModerationRule{
			TriggerMetadata: &guild.AutoModerationTriggerMetadata{
				KeywordFilter: []string{"cat"},
			},
			Actions: []guild.AutoModerationAction{
		...
		if err != nil {
			session.GuildAPI().AutoModerationRuleDelete(ctx, *GuildID, rule.ID)
			panic(err)
		}

		s.ChannelAPI().MessageSend(ctx, e.ChannelID, "Congratulations! You have just triggered an auto moderation rule.\n"+
		...
		session.EventManager().AddHandler(func(s bot.Session, e *event.AutoModerationActionExecution) {
			...
			if counter == 1 {
				counterMutex.Unlock()
				s.ChannelAPI().MessageSend(ctx, e.ChannelID, "Nothing has changed, right? "+
				...
				counterMutex.Unlock()
				s.ChannelAPI().MessageSend(ctx, e.ChannelID, "Now the second ("+action+") action got executed.")
				s.ChannelAPI().MessageSend(ctx, e.ChannelID, "And... you've made it! That's the end of the example.\n"+
